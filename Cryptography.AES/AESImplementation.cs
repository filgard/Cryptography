using System;
using System.Collections.Generic;

namespace Cryptography.AES
{
    public static class AESImplementation
    {

        private static int Nb, Nk, Nr;
        private static sbyte[][] w;  
        public static List<string> fileContent = new List<string>(); 

        public static int[] sbox = new int[]
        {};

        public static int[] inv_sbox = new int[] {
         };


        public static int[] Rcon = new int[] {
         };

        public static sbyte[] xor_func(sbyte[] a, sbyte[] b)
        {
            sbyte[] output = new sbyte[a.Length];

            for (int i = 0; i < a.Length; i++)
            {
                output[i] = (sbyte)(a[i] ^ b[i]);
            }

            return output;
        }
        private static sbyte[][] generateSubkeys(sbyte[] key)
        {
            sbyte[][] tmp = RectangularArrays.ReturnRectangularSbyteArray(Nb * (Nr + 1), 4);
            int i = 0;

            while (i < Nk)
            {
                tmp[i][0] = key[i * 4];
                tmp[i][1] = key[i * 4 + 1];
                tmp[i][2] = key[i * 4 + 2];
                tmp[i][3] = key[i * 4 + 3];
                i++;
            }
            i = Nk;

            while (i < Nb * (Nr + 1))
            {
                sbyte[] temp = new sbyte[4];
                for (int k = 0; k < 4; k++)
                {
                    temp[k] = tmp[i - 1][k];
                }

                if (i % Nk == 0)
                {
                    temp = SubWord(rotateWord(temp));
                    temp[0] = (sbyte)(temp[0] ^ (Rcon[i / Nk] & 0xff));
                }
                else if (Nk > 6 && i % Nk == 4)
                {
                    temp = SubWord(temp);
                }
                tmp[i] = xor_func(tmp[i - Nk], temp);
                i++;
            }

            return tmp;
        }
        public static sbyte[] SubWord(sbyte[] input)
        {
            sbyte[] tmp = new sbyte[input.Length];

            for (int i = 0; i < tmp.Length; i++)
            {
                tmp[i] = unchecked((sbyte)(sbox[input[i] & 0x000000ff] & 0xff));
            }

            return tmp;
        }
        public static sbyte[] rotateWord(sbyte[] input)
        {
            sbyte[] tmp = new sbyte[input.Length];
            tmp[0] = input[1];
            tmp[1] = input[2];
            tmp[2] = input[3];
            tmp[3] = input[0];

            return tmp;
        }
       
        public static sbyte[][] AddRoundKey(sbyte[][] state, sbyte[][] w, int round)
        {
            sbyte[][] tmp = RectangularArrays.ReturnRectangularSbyteArray(state.Length, state[0].Length);

            for (int c = 0; c < Nb; c++)
            {
                for (int l = 0; l < 4; l++)
                {
                    tmp[l][c] = (sbyte)(state[l][c] ^ w[round * Nb + c][l]);
                }
            }

            return tmp;
        }
        public static sbyte[][] SubBytes(sbyte[][] state)
        {
            sbyte[][] tmp = RectangularArrays.ReturnRectangularSbyteArray(state.Length, state[0].Length);

            for (int row = 0; row < 4; row++)
            {
                for (int col = 0; col < Nb; col++)
                {
                    tmp[row][col] = unchecked((sbyte)(sbox[(state[row][col] & 0x000000ff)] & 0xff));
                }
            }

            return tmp;
        }
        public static sbyte[][] InvSubBytes(sbyte[][] state)
        {
            for (int row = 0; row < 4; row++)
            {
                for (int col = 0; col < Nb; col++)
                {
                    state[row][col] = unchecked((sbyte)(inv_sbox[(state[row][col] & 0x000000ff)] & 0xff));
                }
            }

            return state;
        }
        private static sbyte[][] ShiftRows(sbyte[][] state)
        {
            sbyte[] t = new sbyte[4];
            for (int r = 1; r < 4; r++)
            {
                for (int c = 0; c < Nb; c++)
                {
                    t[c] = state[r][(c + r) % Nb];
                }
                for (int c = 0; c < Nb; c++)
                {
                    state[r][c] = t[c];
                }
            }

            return state;
        }
        public static sbyte[][] InvShiftRows(sbyte[][] state)
        {
            sbyte[] t = new sbyte[4];
            for (int r = 1; r < 4; r++)
            {
                for (int c = 0; c < Nb; c++)
                {
                    t[(c + r) % Nb] = state[r][c];
                }
                for (int c = 0; c < Nb; c++)
                {
                    state[r][c] = t[c];
                }
            }
            return state;
        }
        public static sbyte[][] InvMixColumns(sbyte[][] s)
        {
            int[] sp = new int[4];
            sbyte b02 = (sbyte)0x0e, b03 = (sbyte)0x0b, b04 = (sbyte)0x0d, b05 = (sbyte)0x09;

            for (int c = 0; c < 4; c++)
            {
                sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04, s[2][c]) ^ FFMul(b05, s[3][c]);
                sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ FFMul(b04, s[3][c]);
                sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
                sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05, s[2][c]) ^ FFMul(b02, s[3][c]);
                for (int i = 0; i < 4; i++)
                {
                    s[i][c] = (sbyte)(sp[i]);
                }
            }

            return s;
        }
        public static sbyte[][] MixColumns(sbyte[][] s)
        {
            int[] sp = new int[4];
            sbyte b02 = (sbyte)0x02, b03 = (sbyte)0x03;
            for (int c = 0; c < 4; c++)
            {
                sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c] ^ s[3][c];
                sp[1] = s[0][c] ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
                sp[2] = s[0][c] ^ s[1][c] ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
                sp[3] = FFMul(b03, s[0][c]) ^ s[1][c] ^ s[2][c] ^ FFMul(b02, s[3][c]);
                for (int i = 0; i < 4; i++)
                {
                    s[i][c] = (sbyte)(sp[i]);
                }
            }

            return s;
        }
        public static sbyte FFMul(sbyte a, sbyte b)
        {
            sbyte aa = a, bb = b, r = 0, t;
            while (aa != 0)
            {
                if ((aa & 1) != 0)
                {
                    r = (sbyte)(r ^ bb);
                }
                t = unchecked((sbyte)(bb & 0x80));
                bb = (sbyte)(bb << 1);
                if (t != 0)
                {
                    bb = (sbyte)(bb ^ 0x1b);
                }
                aa = (sbyte)((aa & 0xff) >> 1);
            }
            return r;
        }
        public static sbyte[] encryptBloc(sbyte[] input)
        {
            sbyte[] tmp = new sbyte[input.Length];
            sbyte[][] state = RectangularArrays.ReturnRectangularSbyteArray(4, Nb);

            for (int i = 0; i < input.Length; i++)
            {
                state[i / 4][i % 4] = input[i % 4 * 4 + i / 4];
            }

            state = AddRoundKey(state, w, 0);
            FileHelper.GetDescription("start round initialisation from 1 to " + Nr);
            for (int round = 1; round < Nr; round++)
            {
                FileHelper.GetDescription("round: {0}", round.ToString());
                state = SubBytes(state);
                FileHelper.GetDescription("{0,18}", "SubBytes state");
                FileHelper.GetMatrixState(state);

                state = ShiftRows(state);
                FileHelper.GetDescription("{0, 19}", "ShiftRows state");
                FileHelper.GetMatrixState(state);

                state = MixColumns(state);
                FileHelper.GetDescription("{0, 20}", "MixColumns state");
                FileHelper.GetMatrixState(state);

                state = AddRoundKey(state, w, round);
                FileHelper.GetDescription("{0, 21}", "AddRoundKey state");
                FileHelper.GetMatrixState(state);
            }
            FileHelper.GetDescription("round: {0}", "10");
            state = SubBytes(state);
            FileHelper.GetDescription("{0,18}", "SubBytes state");
            FileHelper.GetMatrixState(state);

            state = ShiftRows(state);
            FileHelper.GetDescription("{0, 19}", "ShiftRows state");
            FileHelper.GetMatrixState(state);

            state = AddRoundKey(state, w, Nr);
            FileHelper.GetDescription("{0, 21}", "AddRoundKey state");
            FileHelper.GetMatrixState(state);

            for (int i = 0; i < tmp.Length; i++)
            {
                tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
            }

            return tmp;
        }
        public static sbyte[] decryptBloc(sbyte[] input)
        {
            sbyte[] tmp = new sbyte[input.Length];
            sbyte[][] state = RectangularArrays.ReturnRectangularSbyteArray(4, Nb);

            for (int i = 0; i < input.Length; i++)
            {
                state[i / 4][i % 4] = input[i % 4 * 4 + i / 4];
            }

            state = AddRoundKey(state, w, Nr);

            FileHelper.GetDescription("start round initialisation from " + (Nr - 1) + " to " + 1);
            for (int round = Nr - 1; round >= 1; round--)
            {
                FileHelper.GetDescription("round: {0}", round.ToString());
                state = InvSubBytes(state);
                FileHelper.GetDescription("{0,18}", "InvSubBytes state");
                FileHelper.GetMatrixState(state);

                state = InvShiftRows(state);
                FileHelper.GetDescription("{0, 19}", "InvShiftRows state");
                FileHelper.GetMatrixState(state);

                state = AddRoundKey(state, w, round);
                FileHelper.GetDescription("{0, 21}", "AddRoundKey state");
                FileHelper.GetMatrixState(state);

                state = InvMixColumns(state);
                FileHelper.GetDescription("{0, 20}", "InvMixColumns state");
                FileHelper.GetMatrixState(state);
            }
            FileHelper.GetDescription("round: {0}", "0");
            state = InvSubBytes(state);
            FileHelper.GetMatrixState(state);

            state = InvShiftRows(state);
            FileHelper.GetDescription("{0, 19}", "InvShiftRows state");
            FileHelper.GetMatrixState(state);

            state = AddRoundKey(state, w, 0);
            FileHelper.GetDescription("{0, 21}", "AddRoundKey state");
            FileHelper.GetMatrixState(state);

            for (int i = 0; i < tmp.Length; i++)
            {
                tmp[i % 4 * 4 + i / 4] = state[i / 4][i % 4];
            }

            return tmp;
        }
        public static sbyte[] encrypt(sbyte[] input, sbyte[] key)
        {
            FileHelper.GetDescription("Encryption start");
            Nb = 4;
            Nk = key.Length / 4;
            Nr = Nk + 6;

            int lenght = 0;
            sbyte[] padding = new sbyte[1];
            int i;

            lenght = 16 - input.Length % 16;
            padding = new sbyte[lenght];
            padding[0] = unchecked((sbyte)0x80);

            for (i = 1; i < lenght; i++)
            {
                padding[i] = 0;
            }

            sbyte[] tmp = new sbyte[input.Length + lenght];
            sbyte[] bloc = new sbyte[16];

            w = generateSubkeys(key);
            FileHelper.GetDescription("Generated Sub Keys");
            FileHelper.GetKeyMatrix(w);


            int count = 0;

            for (i = 0; i < input.Length + lenght; i++)
            {
                if (i > 0 && i % 16 == 0)
                {
                    FileHelper.GetDescription("Take the current 16 bytes from input text");
                    bloc = encryptBloc(bloc);
                    Array.Copy(bloc, 0, tmp, i - 16, bloc.Length);
                }
                if (i < input.Length)
                {
                    bloc[i % 16] = input[i];
                }
                else
                {
                    bloc[i % 16] = padding[count % 16];
                    count++;
                }
            }

            if (bloc.Length == 16)
            {
                FileHelper.GetDescription("bloc.Length = 16");
                bloc = encryptBloc(bloc);
                Array.Copy(bloc, 0, tmp, i - 16, bloc.Length);
            }

            return tmp;
        }
        public static sbyte[] decrypt(sbyte[] input, sbyte[] key)
        {
            FileHelper.GetDescription("Decryption start");
            int i;
            sbyte[] tmp = new sbyte[input.Length];
            sbyte[] bloc = new sbyte[16];
            Nb = 4;
            Nk = key.Length / 4;
            Nr = Nk + 6;

            w = generateSubkeys(key);
            FileHelper.GetDescription("Generated Sub Keys");
            FileHelper.GetKeyMatrix(w);

            for (i = 0; i < input.Length; i++)
            {
                if (i > 0 && i % 16 == 0)
                {
                    FileHelper.GetDescription("Take the current 16 bytes from cipher text");
                    bloc = decryptBloc(bloc);
                    Array.Copy(bloc, 0, tmp, i - 16, bloc.Length);
                }
                if (i < input.Length)
                {
                    bloc[i % 16] = input[i];
                }
            }
            FileHelper.GetDescription("Last decryption block");
            bloc = decryptBloc(bloc);
            Array.Copy(bloc, 0, tmp, i - 16, bloc.Length);

            tmp = deletePadding(tmp);

            return tmp;
        }
        private static sbyte[] deletePadding(sbyte[] input)
        {
            int count = 0;
            int i = input.Length - 1;
            while (input[i] == 0)
            {
                count++;
                i--;
            }

            sbyte[] tmp = new sbyte[input.Length - count - 1];
            Array.Copy(input, 0, tmp, 0, tmp.Length);
            return tmp;
        }
    }
}
